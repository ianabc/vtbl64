...
void disp_srec(int ndx,int w,unsigned char *def, unsigned char *dat)
{
    ...
    {
       if(sd->len == 3)
	  printf("\n error length of %d for type %c field",ndx,sd->len,sd->type);
       else if(sd->len == 4)
          n = *((long *)ch);
       else if(sd->len == 2)
...
int parse_dbf(int ndbf, unsigned char mode,long targ,int *fh)
{
   ...
   fld = mode & FLDMSK;
   printf("\n%s with %d fields",dbf_files[ndbf].name,dbf_files[ndbf].nfields-1);
   ...
   {
      printf("read error obtaining structure definition, read %d bytes\n",rd);
      return(-2);
   }
   ...
   for(i=1,len = 32;i<dbf_files[ndbf].nfields;i++,len+=32)
   {
       ...
       if(mode & DISPFLD) // mimic old logic where always display fields
          printf("\n%2d %-10s  %c    0x%03x  %02d",i,sd->fname,sd->type,sd->off,sd->len);
   ...
   if(buf[len] != 0xD)
   {
       printf("\nFailed to find structure terminator at file offset 0x%x",len);
       return(-3);
   }
   ...
   {
       printf("\nBUFSZ < %d bytes, not enough room to buffer data",len+rd);
       return(-4);
   }
   ...
   {
       printf("\ndata read error for entry %d, no data terminator found",cnt+1);
...
{
   int cnt=0,fcnt=0,ff,fmax,fp,i,ldsk=-1,len,rd,ret,tape_id=-1;
   ...
   */
   ...
   if(ret < 1)
   {
       printf("\nabort: did not load 'volumes.dbf' rec containing ID = %d",vol_id);
       return(-1);
   }
   if(fp > 0)
   ...
   printf("\nTape ID:     "); disp_srec(2,-1,buf,dat);
   printf("\nDescription: "); disp_srec(5,-1,buf,dat);
   printf("\nTIME_DATE:   "); disp_srec(6,-1,buf,dat);
   printf("\nCompression: "); disp_srec(13,-1,buf,dat);
   printf("\nData size:   "); disp_srec(20,-1,buf,dat);
   printf("\nDir size:    "); disp_srec(22,-1,buf,dat);
   ...
   {
       printf("\nunable to load 'tapes.dbf' record with 'ID' = %d",tape_id);
   ...
   else
   {
      ...
      dat = buf+len;
      printf("\nID:          "); disp_srec(1,-1,buf,dat);
      printf("\nNAME:        "); disp_srec(2,-1,buf,dat); // job # and last disk #
      printf("\nLFTIME:      "); disp_srec(4,-1,buf,dat); // 1st of 4 time stamps
      tstr = dat+0x3b; // known offset to LFTIME
      t = tfld2time_t(tstr);
      if((long)t > 0 && (long)vt > 0 && ((long)(t - vt) > 300 || (long)(t -vt) < -300) )
...
*/
               disp_srec(3,14,fimg,fdat+dbf_files[1].ent_sz * (fid -1));
	 }
	 if(mode & DISPDAT)
	 {
   ...
   printf("\n%d have associated data with %ld total data bytes",fcnt,cflen);
   if(fp > 0)
      close(fp); // done with this file
   return(cnt);
}
...
{  
   ...
   {
       printf("\nabort: unable to open and load '%s' record 1",dbf_files[ndbf].name);
       return(-4);
   }
...
void main(int argc, char *argv[])
{
   ...
   else if((*argv[1] == 'b' || *argv[1] == 'B') && sscanf(argv[1]+1,"%d",&i) == 1)
   {
          disp_bkup(i,mode);
   }
