...
*/
BYTE *get_fheader(FILE * fp, int mode)
{
    BYTE *hdr = NULL;
    DWORD *sig;
    int rd = 0;
    if ((hdr = (unsigned char *) malloc(HDR_SZ)) == NULL ||
        fread(hdr, HDR_SZ, 1, fp) != HDR_SZ) {
        if (hdr == NULL)
...
*/
int parse_comp(FILE * fp, char mode, char *hdr)
{
    long cnt = 1, off, len;
    ...
    while (ret == 0) {
        if (fread(ibuf, BLK_SZ, 1, fp) != BLK_SZ) {
            printf("\nblock read error");
            ret = -2;
...
struct uni_str *get_unistr(FILE * fp)
{
    WORD rd, w;
    struct uni_str *us = NULL;
    if ((rd = fread(&w, 2, 1, fp)) == 2 &&
        (us = (struct uni_str *) malloc((int) w + 2)) != NULL) {
        us->len = w;            // string length
        if ((rd = fread((char *) us + 2, w, 1, fp)) != w) {
            free(us);
            us = NULL;
        }
    }
...
*/
int get_dheader(FILE * fp, struct dir_head *dh)
{
    ...
    if ((rd = fread((BYTE *) dh, 73, 1, fp)) != 73
        || (tsig = *((DWORD *) dh)) != DIR_SIG) {
        if (rd == 73 && tsig == 0)
            return (1);         // valid EOF
        else {
            printf("\ninvalid header");
        ...
        ret = -2;
    else if ((rd = fread((BYTE *) & dh->unknw2, 21, 1, fp)) != 21)
        ret = -1;
    else if ((dh->nm2 = get_unistr(fp)) == NULL)
        ret = -2;
    else if ((rd = fread((BYTE *) & dh->unknw3, 28, 1, fp)) != 28)
        ret = -1;
    else if ((dh->nm3 = get_unistr(fp)) == NULL)
        ret = -2;
    else if ((rd = fread(buf, 2, 1, fp)) != 2)  // should be word 0xA that follows 3rd uni_str
        ret = -1;
    else
...
int get_cheader(FILE * fp, struct cat_head *ch)
{
    int rd, ret = 0;
    ch->nm1 = ch->nm2 = ch->nm3 = NULL;

    if ((rd = fread((BYTE *) ch, 69, 1, fp)) != 69) {
    ...
    else if ((rd = fread((BYTE *) & ch->unknw2, 21, 1, fp)) != 21)
        ret = -1;
    else if ((ch->nm2 = get_unistr(fp)) == NULL) {
        free(ch->nm1);
        ch->nm1 = NULL;
        ret = -2;
    } else if ((rd = fread((BYTE *) & ch->unknw3, 28, 1, fp)) != 28)
        ret = -1;
...
int
do_extract(FILE * fp, struct uni_str *nm, long *len, time_t * t,
           BYTE atrib)
{
    int fo, rd, ret = 0;
    char *ch;
        ...
        *ch = 0;
        ...
        while (ret == 0 && wr < *len)   // copy file data
        {
            if (*len - wr > BUF_SZ)
                ...
                rd = *len - wr;
            if (fread(buf, rd, 1, fp) != rd || write(fo, buf, rd) != rd) {
                ret = 3;
                printf("\nfile io error");
            } else
                wr += rd;
...
int
parse_ucomp(FILE * fp, char mode, long soff, struct dir_list **ptrl,
            char *xpath)
{
...
//  printf("\ntimestamps");
        if (verb) {
            ...
            printf("\n  1: ctime() of value 0x%lx: %s", (long) *t,
                   ctime(t));
        } else
            printf("\n     ctime() of value 0x%lx: %s", (long) *t,
                   ctime(t));

        if (verb) {
            for (i = 2; i < 4; i++) {
                t += 2;         // every 8 bytes
                printf("%3d: ctime() of value 0x%lx: %s", i, (long) *t,
                       ctime(t));
            }
        ...
        else if (mode & XTRACT) // flen is 0 when its a drive or dir spec
        {
            ...
            if (xpath == NULL || (j > 0 && strncmp(xpath, buf, j) == 0)) {
                ...
                if (do_extract(fp, dhead.nm1, flen, t, dhead.unknw1[41]) ==
                    0) {
...
*/
int parse_cat(FILE * fp, char mode, struct cat_list **ptrl, long off)
{
    int cnt = 0, i, ret = -1;
...
*/
void
init_tests(BYTE unknwn[], BYTE bconst[], BYTE unknw1[], BYTE unknw2[],
           BYTE unknw3[])
{
    int i, j;
...
int
test_rec(BYTE unknwn[], BYTE bconst[], BYTE unknw1[], BYTE unknw2[],
         BYTE unknw3[])
{
    int i, j, mcnt;
    mcnt = 0;
...
*/
long find_sig(FILE * fp, long loff, unsigned long sig)
{
    int i, len, rd, ret = 0;
...
int main(int argc, char *argv[])
{
    ...
    if (!fp) {
        ...
        nblk = *((long *) (hdr + 0xa));
        off = 0x7400 * (nblk - 1);
        printf("\nblock count from header %ld => catalog at offset 0x%lx",
               nblk, off);
    ...
    if (ret != 0)
        ...
        ret = -3;
    } else if (fread(buf, 8, 1, fp) != 8) {
        printf("\nfailed to read 8 bytes of UCOMP data region at 0x%x",
               START_DATA);
        ret = -4;
