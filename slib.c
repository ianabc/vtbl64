...
int main(int argc, char *argv[])
{
   ...
   printf("\nslib Version 1.00 compiled for %s using data file: %s",OS_STR,argv[1]);
   ...
   else
   {
      doff = *((long *)(buf+0x29));
      off = *((long *)(buf+0x33));
      printf("\noffset to catalog header 0x%lx and name list region 0x%lx",doff,off);
...
// confirmed flen below.  doff probably correct
	  ...
	  doff = *((long *)(buf+11)); // offset to start data
          ...
          else
          {
             *(ch+nlen) = 0;
             printf("\n%2d: %-25s @ 0x%06lx len %8ld %02d-%02d-%4d %02d:%02d",
		      cnt++,ch,off,flen,ptrd->mon,ptrd->day,ptrd->yr+1980,
		      ptrt->hr,ptrt->min);
	        ...
	        if(mode & VERBOSE)
		{
		    ...
		    printf("\n0x%02x:",0x1E + nlen);
	            for(j=0;j<13;j++)
  	               printf(" 0x%02x",*((unsigned char *)(ch+j)));
                    putchar('\n');
		 }
                 off += 13; 
	     }
