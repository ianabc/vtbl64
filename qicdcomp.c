...
10/26/10 was running very slow on decompress of large files.
   played with debug messages some, tried adding
   param to decomp_seg(char disp) to toggle at run time.
   don't like it, can't pass flag to do_decompress() easily.
   instead add another ugly global, dcomp_verb, which replaces
   verb in decom_seg() and remove parameter I added

02/24/19  copy this version of source code from csource/dump/msbackup.
   The file date was 10/26/10 at 1:44 pm  its the version I have been using,
   I want to play with code some for vtbl.c and iomega *.113 files.  Do not 
   want to change original source I distributed so do new work here.
   
   change this to include local qicdcomp.h rather than original msqic.h
   this requires I add MSQIC conditional so top level routine do_decompress()
   is only compiled when MSQIC defined.
   It uses WINME define and struct vtbl_ver which are not needed for Iomega work.
   Also had to include stdlib.h below to provide a define for exit().
*/
#include <stdio.h>
#include <fcntl.h>
#include <stdlib.h>             // added 02/24/19 for iomega build to define exit()
...
BYTE dcomp_verb = 1;            //default dcomp_verbosity for decomp_seg, global so main can tweak
...
----------*/
/* 10/25/10 add param dcomp_verb, was a compile time thing, now
   has two starts 2 for verything, 1 for default message (old style)
...
void decomp_seg(WORD seg_sz)               // was being sloppy, no return!
{
    ...
    bits = 0;
    bcnt = 0;
    comp_rd = 0, comp_wr = 0;   // init so repeat calls can be made
    ...
    while (hptr < limit)        // or till terminator, pat == 0x180, off == 0
    {
        if (getbit() == 0)      // raw byte
        {
            if (hptr == HBUF_SZ)
                flush_hbuf();
            if (rawndx < 0)     // always track this, use dcomp_verb to control display
...
#ifdef MSQIC                    // compiling for msbackup and have included msqic.h
      // I added logic I wanted in vtbl.c but should review to see if missed anything
...
//  minimize parameter passing by making fin and fout locally global
    ...
    if ((FOFFSET) lseek(fin, cvtbl->database, SEEK_SET) != cvtbl->database) {
        ...
        suc = 2;
    } else if ((FOFFSET) lseek(fout, (FOFFSET) sz, SEEK_SET) !=
               (FOFFSET) sz) {
    ...
    if (suc == 0) {
        ...
        else                    // add RECREAT test to set dirbase, assume next segment
        {
            if (cvtbl->ver & RECREAT) {
                if (cvtbl->dirbase == 0) {
                    ...
                    cvtbl->dirbase =
                        cum_rd + nshead * sizeof(struct cseg_head);
                ...
                if ((pad =
                     get_dir_len(fin, 1, cvtbl->dirbase)) == LSEEK_ERR) {
            ...
            if (suc == 0
                && (cur_pos =
                    lseek(fin, cvtbl->dirbase,
                          SEEK_SET)) != cvtbl->dirbase)
        ...
        if (suc == 0) {
            ...
            cum_wr = 0;
            do {
                if (read(fin, &shead, sizeof(struct cseg_head)) !=
                    sizeof(struct cseg_head))
                    suc = 5;
                    ...
                    cum_wr += comp_wr;
                if (cum_wr >= vtbl.dirSz)
                    break;
            } while (suc == 0);
            // if(cvtbl->ver & RECREAT) don't know dirSz, set from cum_wr
            if (suc == 5
                && (cum_wr == vtbl.dirSz || cvtbl->ver == RECREAT))
